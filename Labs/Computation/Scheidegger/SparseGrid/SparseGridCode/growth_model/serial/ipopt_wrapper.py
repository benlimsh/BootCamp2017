...
def EV_F_ITER(X, k_init, n_agents, grid):
    ...
    VT_sum_vec = np.zeros((5,1))
    for i in range(5):
        VT_sum_vec[i] = utility(cons, lab) + beta*grid[i].evaluate(knext)

    VT_sum = np.sum(VT_sum_vec)/5
...
def EV_G(X, k_init, n_agents, theta):
    ...
    #theta = np.array([0.9, 0.95, 1.0, 1.05, 1.10])
    #f_prod_vec = np.zeros((5,1))
    #for i in range(len(f_prod_vec)):
    #    f_prod_vec[i] = 0.2*theta[i]*output_f(k_init, lab)
    #f_prod=np.sum(f_prod_vec)
    #output_f(k_init, lab)
